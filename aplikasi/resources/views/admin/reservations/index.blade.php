...
<div class="p-6">
            ...
            <tbody>
                ...
                <tr class="border-t">
                    ...
                    <td class="px-3 py-2">
                        ...
                        if ($minutes < 60) return $minutes . 'm' ; $hours=floor($minutes / 60); $mins=$minutes % 60; if
                            ($hours>= 24) {
                            $days = floor($hours / 24);
                            $hours = $hours % 24;
                            if ($hours > 0) {
                            ...
                            <div style="font-size: 11px; line-height: 1.6;">
                                <div style="margin-bottom: 4px;">
                                    ...
                                    <span
                                        style="background: #f3e8ff; color: #6b21a8; padding: 2px 6px; border-radius: 4px; font-weight: 700;">#{{
                                        $r->queue_position ?? '-' }}</span>
                                </div>
                                <div style="margin-bottom: 4px;">
                                    <span style="color: #2563eb; font-weight: 600;">WT:</span>
                                    <span
                                        style="background: #dbeafe; color: #1e40af; padding: 2px 6px; border-radius: 4px;">{{
                                        $formattedWT }}</span>
                                </div>
                                <div style="margin-bottom: 4px;">
                                    <span style="color: #16a34a; font-weight: 600;">TAT:</span>
                                    <span
                                        style="background: #dcfce7; color: #166534; padding: 2px 6px; border-radius: 4px;">{{
                                        $formattedTAT }}</span>
                                </div>
                                <div
                                    style="color: #4b5563; margin-top: 6px; padding-top: 4px; border-top: 1px solid #e5e7eb;">
                    ...
                    <td class="px-3 py-2">
                        @if ($r->request_letter)
                        <a class="text-blue-600 underline" href="{{ Storage::url($r->request_letter) }}"
                    ...
                    <td class="px-3 py-2">
                        ...
                        $endDateTime = \Carbon\Carbon::parse($r->reservation_date->format('Y-m-d') . ' ' .
                        $r->end_time);
                        ...
                        <span class="text-gray-500 font-semibold">⚪ Dibatalkan</span>
                        @else
                    ...
                    <td class="px-3 py-2">
                        ...
                        <form method="POST" action="{{ route('admin.reservations.updateStatus', $r) }}"
                            class="reservation-form" id="form-{{ $r->id }}">
                            @csrf
                            <div class="flex flex-col gap-2">
                                <input type="time" name="visit_time"
                                    value="{{ $r->visit_time ? \Carbon\Carbon::parse($r->visit_time)->format('H:i') : '' }}"
                                    class="border rounded p-1 text-sm" step="300" placeholder="Set waktu" />
                                <select name="status" class="border rounded p-1 text-sm status-select"
                                    data-reservation-id="{{ $r->id }}" data-reservation-name="{{ $r->full_name }}">
                                    <option value="confirmed" @selected($r->status === 'confirmed')>✓ Terima</option>
                                    <option value="rejected">✗ Tolak</option>
                                    <option value="cancelled" @selected($r->status === 'cancelled')>⚪ Batal</option>
                                </select>
                                <textarea name="rejection_reason"
                                    class="border rounded p-1 text-sm rejection-reason-input hidden"
                                    placeholder="Tulis alasan penolakan..." rows="3" maxlength="500"></textarea>
                                <button type="submit" class="btn bg-blue-600"
    ...
    <div class="mt-6"><a href="{{ route('admin.books.index') }}" class="text-blue-600 underline">Kembali ke
            Dashboard</a></div>
</div>
...
<div id="rejection-modal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
        ...
        <p class="mb-4 text-gray-700">Anda akan menolak reservasi dari <strong id="modal-reservation-name"></strong>.
        </p>
        <p class="mb-4 text-sm text-gray-600">Silakan tulis alasan penolakan agar user dapat memahami keputusan ini:</p>
        <textarea id="modal-rejection-reason" class="w-full border rounded p-3 text-sm mb-4"
            placeholder="Contoh: Waktu bentrok dengan reservasi lain yang sudah dikonfirmasi" rows="4"
        ...
        <div class="flex gap-2 justify-end">
            ...
            <button id="modal-confirm" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Tolak
                Reservasi</button>
        </div>
    </div>
</div>
